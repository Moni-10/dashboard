...
//#endregion
//#region node_modules/tinymce/plugins/visualchars/index.js
	...
	(function() {
		...
		*/
		...
		const remove$2 = (element, attr, id) => {
			...
			if (nu.length > 0) set(element, attr, nu.join(" "));
		...
		const cleanClass = (element) => {
			if ((supports(element) ? element.dom.classList : get(element)).length === 0) remove$3(element, "class");
		};
		...
		const show = (editor, rootElm) => {
			...
			each$1(nodeList, (n) => {
				...
				if (isWrappedNbsp(parent)) add(SugarElement.fromDom(parent), nbspClass);
					...
					let node;
					while (node = div.lastChild) dom.insertAfter(node, n.dom);
		...
		const applyVisualChars = (editor, toggleState) => {
			...
			const body = editor.getBody();
			if (toggleState.get() === true) show(editor, body);
			else hide(editor, body);
		};
