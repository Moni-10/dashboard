...
*/
//#endregion
//#region node_modules/tinymce/plugins/codesample/index.js
	...
	*/
	(function() {
		...
		*/
		...
		const prismjs = function(global, module$1, exports$1) {
			...
			if (typeof module$1 !== "undefined" && module$1.exports) module$1.exports = Prism;
...
value: function(attrName, lang) {
		...
		const getCurrentCode = (editor) => {
			return getSelectedCodeSample(editor).bind((n) => Optional.from(n.textContent)).getOr("");
		};
		...
		const getCurrentLanguage = (editor, fallback) => {
			return getSelectedCodeSample(editor).fold(() => fallback, (n) => {
		...
		const register$1 = (editor) => {
			editor.addCommand("codesample", () => {
				...
				if (editor.selection.isCollapsed() || isCodeSample(node)) open(editor);
				else editor.formatter.toggle("code");
			});
		};
		...
		const setup = (editor) => {
			editor.on("PreProcess", (e) => {
				...
				global.each(global.grep(pres, isCodeSample), (elm) => {
					...
					let child;
					while (child = elm.firstChild) elm.removeChild(child);
			...
			editor.on("SetContent", () => {
				...
				if (unprocessedCodeSamples.length) editor.undoManager.transact(() => {
			...
			editor.on("PreInit", () => {
				editor.parser.addNodeFilter("pre", (nodes) => {
					for (let i = 0, l = nodes.length; i < l; i++) {
						const node = nodes[i];
						if ((node.attr("class") ?? "").indexOf("language-") !== -1) {
		...
		var Plugin = () => {
			global$2.add("codesample", (editor) => {
				...
				editor.on("dblclick", (ev) => {
					if (isCodeSample(ev.target)) open(editor);
				});
			});
		};
		Plugin();
