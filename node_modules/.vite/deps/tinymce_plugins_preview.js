...
//#endregion
//#region node_modules/tinymce/plugins/preview/index.js
	...
	(function() {
		...
		*/
		...
		const getPreventClicksOnLinksScript = () => {
			...
			const fn = (isMacOSOrIOS) => {
				document.addEventListener("click", (e) => {
					for (let elm = e.target; elm; elm = elm.parentNode) if (elm.nodeName === "A") {
						...
						if (!(isMacOSOrIOS ? e.metaKey : e.ctrlKey && !e.altKey)) e.preventDefault();
					}
				}, false);
			};
		...
		const getStyleSheetCrossOrigin = (editor) => {
			if (shouldUseContentCssCors(editor)) return constant("anonymous");
		...
		const getPreviewHtml = (editor, contentCssResources) => {
			...
			global.each(contentCssResources, (resource) => {
				if (resource.type === "bundled") headHtml += "<style type=\"text/css\">" + resource.content + "</style>";
				else {
			...
			if (contentStyle) headHtml += "<style type=\"text/css\">" + contentStyle + "</style>";
			headHtml += getComponentScriptsHtml(editor);
			const bodyId = getBodyId(editor);
			const bodyClass = getBodyClass(editor);
			const directionality = editor.getBody().dir;
			const dirAttr = directionality ? " dir=\"" + encode(directionality) + "\"" : "";
			return "<!DOCTYPE html><html><head>" + headHtml + "</head><body id=\"" + encode(bodyId) + "\" class=\"mce-content-body " + encode(bodyClass) + "\"" + dirAttr + ">" + editor.getContent() + getPreventClicksOnLinksScript() + "</body></html>";
		};
		...
		var Plugin = () => {
			global$2.add("preview", (editor) => {
				const getContentCssResources = () => map(editor.contentCSS, (key) => Optional.from(tinymce.Resource.get(key)).filter(isString).map((content) => ({
					type: "bundled",
					content
				})).getOr({
					type: "link",
					url: editor.documentBaseURI.toAbsolute(key)
				}));
