...
//#endregion
//#region node_modules/tinymce/plugins/nonbreaking/index.js
	...
	*/
	(function() {
		...
		const stringRepeat = (string, repeats) => {
			let str = "";
			for (let index = 0; index < repeats; index++) str += string;
			return str;
		};
		...
		const insertNbsp = (editor, times) => {
			...
			const html = wrapNbsps(editor) || editor.plugins.visualchars ? nbspSpan() : stringRepeat("&nbsp;", times);
		...
		const setup = (editor) => {
			...
			if (spaces > 0) editor.on("keydown", (e) => {
