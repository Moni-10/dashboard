		...
		*/
		...
		const composeAlphabeticBase26 = (value) => {
			...
			if (value < 0) return "";
				...
				return composeAlphabeticBase26(quotient) + String.fromCharCode("A".charCodeAt(0) + remainder);
			}
		};
		...
		const deduceListType = (start) => {
			...
			else if (isEmpty(start)) return 3;
			else return 4;
		};
		...
		const parseDetail = (detail) => {
			...
			if (is(detail.listStyleType, "upper-alpha")) return composeAlphabeticBase26(start);
			else if (is(detail.listStyleType, "lower-alpha")) return composeAlphabeticBase26(start).toLowerCase();
		...
		const getClosestListHost = (editor, elm, isCollapsed) => {
			...
			return find(parentBlocks, (elm) => (!isCollapsed || isNotForcedRootBlock(elm)) && isListHost(editor.schema, elm)).getOr(editor.getBody());
		};
