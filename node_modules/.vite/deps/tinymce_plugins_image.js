...
//#endregion
//#region node_modules/tinymce/plugins/image/index.js
	...
	*/
	(function() {
		...
		*/
		...
		isFunction(Array.from) && Array.from;
		const findMap = (arr, f) => {
		...
		const deep = (old, nu) => {
			return isPlainObject(old) && isPlainObject(nu) ? deepMerge(old, nu) : nu;
		};
		const baseMerge = (merger) => {
			return (...objects) => {
				...
				for (let j = 0; j < objects.length; j++) {
					...
					for (const key in curObject) if (has(curObject, key)) ret[key] = merger(ret[key], curObject[key]);
				}
				return ret;
			};
		};
		...
		const addPixelSuffix = (value) => {
			if (value.length > 0 && /^[0-9]+$/.test(value)) value += "px";
			return value;
		};
		...
		const createImageList = (editor, callback) => {
			...
			if (isString(imageList)) fetch(imageList).then((res) => {
				if (res.ok) res.json().then(callback);
			});
			else if (isFunction(imageList)) imageList(callback);
			else callback(imageList);
		};
		const waitLoadImage = (editor, data, imgElm) => {
			...
			imgElm.onload = () => {
				if (!data.width && !data.height && hasDimensions(editor)) editor.dom.setAttribs(imgElm, {
		...
		const getHspace = (image) => {
			if (image.style.marginLeft && image.style.marginRight && image.style.marginLeft === image.style.marginRight) return removePixelSuffix(image.style.marginLeft);
			else return "";
		};
		const getVspace = (image) => {
			if (image.style.marginTop && image.style.marginBottom && image.style.marginTop === image.style.marginBottom) return removePixelSuffix(image.style.marginTop);
			else return "";
		};
		const getBorder = (image) => {
			if (image.style.borderWidth) return removePixelSuffix(image.style.borderWidth);
			else return "";
		};
		const getAttrib = (image, name) => {
			if (image.hasAttribute(name)) return image.getAttribute(name) ?? "";
			else return "";
		};
		const hasCaption = (image) => image.parentNode !== null && image.parentNode.nodeName === "FIGURE";
		const updateAttrib = (image, name, value) => {
			if (value === "" || value === null) image.removeAttribute(name);
		...
		const toggleCaption = (image) => {
			if (hasCaption(image)) removeFigure(image);
			else wrapInFigure(image);
		};
		...
		const setSize = (name, normalizeCss) => (image, name, value) => {
			...
			} else updateAttrib(image, name, value);
		};
		const getSize = (image, name) => {
			if (image.style[name]) return removePixelSuffix(image.style[name]);
			else return getAttrib(image, name);
		};
		...
		const getIsDecorative = (image) => {
			...
			return image.hasAttribute("alt") && alt.length === 0 || role === "presentation" || role === "none";
		};
		const getAlt = (image) => {
			if (getIsDecorative(image)) return "";
			else return getAttrib(image, "alt");
		};
		const defaultData = () => ({
			src: "",
			alt: "",
		...
		const getStyleValue = (normalizeCss, data) => {
			...
			if (getHspace(image) || data.hspace !== "") setHspace(image, data.hspace);
			if (getVspace(image) || data.vspace !== "") setVspace(image, data.vspace);
			if (getBorder(image) || data.border !== "") setBorder(image, data.border);
			if (getBorderStyle(image) || data.borderStyle !== "") setBorderStyle(image, data.borderStyle);
		...
		const updateProp = (image, oldData, newData, name, set) => {
			if (newData[name] !== oldData[name]) set(image, name, String(newData[name]));
		};
		const setAlt = (image, alt, isDecorative) => {
			if (isDecorative) {
				...
				if (DOM.getAttrib(image, "role") === "presentation") DOM.setAttrib(image, "role", "");
			}
		};
		const updateAlt = (image, oldData, newData) => {
			if (newData.alt !== oldData.alt || newData.isDecorative !== oldData.isDecorative) setAlt(image, newData.alt, newData.isDecorative);
		};
		...
		const getSelectedImage = (editor) => {
			...
			if (figureElm) return editor.dom.select("img", figureElm)[0];
			if (imgElm && (imgElm.nodeName !== "IMG" || isPlaceholderImage(imgElm))) return null;
			return imgElm;
		};
		const splitTextBlock = (editor, figure) => {
			...
			if (textBlock) return dom.split(textBlock, figure) ?? figure;
			else return figure;
		};
		...
		const insertImageAtCaret = (editor, data) => {
			...
			} else editor.selection.select(insertedElm);
		};
		...
		const insertOrUpdateImage = (editor, partialData) => {
			...
			if (image) {
				...
				if (data.src) writeImageDataToSelection(editor, sanitizedData);
				else deleteImage(editor, image);
			} else if (partialData.src) insertImageAtCaret(editor, {
				...defaultData(),
				...partialData
			});
		};
		...
		const sanitizer = (extractor = getValue) => (list) => {
			if (list) return Optional.from(list).map((list) => sanitizeList(list, extractor));
			else return Optional.none();
		};
		...
		const findEntryDelegate = (list, value) => findMap(list, (item) => {
			if (isGroup(item)) return findEntryDelegate(item.items, value);
			else if (item.value === value) return Optional.some(item);
			else return Optional.none();
		});
		...
		const addPrependUrl2 = (info, srcURL) => {
			if (!/^(?:[a-zA-Z]+:)?\/\//.test(srcURL)) return info.prependURL.bind((prependUrl) => {
				if (srcURL.substring(0, prependUrl.length) !== prependUrl) return Optional.some(prependUrl + srcURL);
				return Optional.none();
			});
			return Optional.none();
		};
		...
		const formFillFromMeta2 = (info, data, meta) => {
			if (info.hasDescription && isString(meta.alt)) data.alt = meta.alt;
			if (info.hasAccessibilityOptions) data.isDecorative = meta.isDecorative || data.isDecorative || false;
			if (info.hasImageTitle && isString(meta.title)) data.title = meta.title;
			if (info.hasDimensions) {
				if (isString(meta.width)) data.dimensions.width = meta.width;
				if (isString(meta.height)) data.dimensions.height = meta.height;
			}
			if (isString(meta.class)) ListUtils.findEntry(info.classList, meta.class).each((entry) => {
			...
			if (info.hasAdvTab) {
				if (isString(meta.style)) data.style = meta.style;
				if (isString(meta.vspace)) data.vspace = meta.vspace;
				if (isString(meta.border)) data.border = meta.border;
				if (isString(meta.hspace)) data.hspace = meta.hspace;
				if (isString(meta.borderstyle)) data.borderstyle = meta.borderstyle;
			}
		};
		...
		const calculateImageSize = (helpers, info, state, api) => {
			...
			if (!meta.width && !meta.height && info.hasDimensions) {
				if (isNotEmpty(url)) helpers.imageSize(url).then((size) => {
					if (state.open) api.setData({ dimensions: size });
				}).catch((e) => console.error(e));
				else api.setData({ dimensions: {
					width: "",
					height: ""
				} });
			}
		...
		const changeImages = (helpers, info, state, api) => {
			...
			image.each((img) => {
				if (data.alt === "" || state.prevImage.map((image) => image.text === data.alt).getOr(false)) {
					if (img.value === "") api.setData({
						src: img,
						alt: state.prevAlt
					});
					else api.setData({
		...
		const changeFileInput = (helpers, info, state, api) => {
			...
			head(data.fileinput).fold(() => {
				...
				blobToDataUri(file).then((dataUrl) => {
					...
					if (info.automaticUploads) helpers.uploadImage(blobInfo).then((result) => {
		...
		const changeHandler = (helpers, info, state) => (api, evt) => {
			if (evt.name === "src") changeSrc(helpers, info, state, api);
			else if (evt.name === "images") changeImages(helpers, info, state, api);
			else if (evt.name === "alt") state.prevAlt = api.getData().alt;
			else if (evt.name === "fileinput") changeFileInput(helpers, info, state, api);
			else if (evt.name === "isDecorative") api.setEnabled("alt", !api.getData().isDecorative);
		};
		const closeHandler = (state) => () => {
			state.open = false;
		};
		const makeDialogBody = (info) => {
			if (info.hasAdvTab || info.hasUploadUrl || info.hasUploadHandler) return {
				type: "tabpanel",
				tabs: flatten([
		...
		const imageSize = (editor) => (url) => {
			if (!isSafeImageUrl(editor, url)) return Promise.resolve({
				width: "",
				height: ""
			});
			else return getImageSize(editor.documentBaseURI.toAbsolute(url)).then((dimensions) => ({
		...
		const uploadImage = (editor) => (blobInfo) => global$1(editor).upload([blobInfo], false).then((results) => {
			if (results.length === 0) return Promise.reject("Failed to upload image");
			else if (results[0].status === false) return Promise.reject(results[0].error?.message);
