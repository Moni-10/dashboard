...
//#endregion
//#region node_modules/tinymce/plugins/quickbars/index.js
		...
		*/
		...
		const pickFile = (editor) => new Promise((resolve) => {
			...
			const cancelHandler = (e) => {
				...
				if (!resolved) {
					if (e.type === "focusin") global.setEditorTimeout(editor, cleanup, 1e3);
					else cleanup();
				}
		...
		const register$1 = (editor) => {
			editor.on("PreInit", () => {
				if (!editor.queryCommandSupported("QuickbarInsertImage")) editor.addCommand("QuickbarInsertImage", () => {
		...
		const addToEditor$1 = (editor) => {
			...
			if (insertToolbarItems.length > 0) editor.ui.registry.addContextToolbar("quickblock", {
				predicate: (node) => {
		...
		const addToEditor = (editor) => {
			...
			if (imageToolbarItems.length > 0) editor.ui.registry.addContextToolbar("imageselection", {
				predicate: isImage,
				items: imageToolbarItems,
				position: "node"
			});
			const textToolbarItems = getTextSelectionToolbarItems(editor);
			if (textToolbarItems.length > 0) editor.ui.registry.addContextToolbar("textselection", {
