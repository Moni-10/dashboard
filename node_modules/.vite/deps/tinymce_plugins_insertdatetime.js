...
//#endregion
//#region node_modules/tinymce/plugins/insertdatetime/index.js
	...
	*/
	(function() {
		...
		const addZeros = (value, len) => {
			value = "" + value;
			if (value.length < len) for (let i = 0; i < len - value.length; i++) value = "0" + value;
			return value;
		};
		...
		const insertDateTime = (editor, format) => {
			if (shouldInsertTimeElement(editor) && editor.selection.isEditable()) {
				...
				let computerTime;
				if (/%[HMSIp]/.test(format)) computerTime = getDateTime(editor, "%Y-%m-%dT%H:%M");
				else computerTime = getDateTime(editor, "%Y-%m-%d");
				const timeElm = editor.dom.getParent(editor.selection.getStart(), "time");
				if (timeElm) updateElement(editor, timeElm, computerTime, userTime);
				else editor.insertContent("<time datetime=\"" + computerTime + "\">" + userTime + "</time>");
			} else editor.insertContent(getDateTime(editor, format));
		};
