...
//#endregion
//#region node_modules/tinymce/plugins/media/index.js
	...
	(function() {
		...
		*/
		...
		const htmlToData = (html, schema) => {
			let data = {};
			const rootNode = global$3({
				validate: false,
				forced_root_block: false
			}, schema).parse(html);
			for (let node = rootNode; node; node = node.walk()) if (node.type === 1) {
				const name = node.name;
				if (node.attr("data-ephox-embed-iri")) {
					data = getEphoxEmbedData(node);
					break;
				} else {
					if (!data.source && name === "param") data.source = node.attr("movie");
					if (name === "iframe" || name === "object" || name === "embed" || name === "video" || name === "audio") {
						if (!data.type) data.type = name;
						data = global$5.extend(node.attributes.map, data);
					}
					if (name === "source") {
						if (!data.source) data.source = node.attr("src");
						else if (!data.altsource) data.altsource = node.attr("src");
					}
					if (name === "img" && !data.poster) data.poster = node.attr("src");
				}
			}
		...
		const updateEphoxEmbed = (data, node) => {
			...
			if (isNonNullable(data.width)) styleMap["max-width"] = addPx(data.width);
			if (isNonNullable(data.height)) styleMap["max-height"] = addPx(data.height);
			...
			for (let node = rootNode; node; node = node.walk()) if (node.type === 1) {
				...
				if (node.attr("data-ephox-embed-iri")) {
					...
					switch (name) {
						...
						case "embed":
						case "img":
						case "iframe": if (data.height !== void 0 && data.width !== void 0) {
							node.attr("width", data.width);
							node.attr("height", data.height);
						}
					}
					if (updateAll) switch (name) {
						case "video":
							node.attr("poster", data.poster);
							node.attr("src", null);
							for (let index = numSources; index < 2; index++) if (data[sources[index]]) {
						...
						case "source":
							...
							sourceCount++;
							break;
						case "img": if (!data.poster) node.remove();
					}
				}
			}
		...
		const getProtocol = (url) => {
			...
			if (protocolMatches && protocolMatches.length > 1) return protocolMatches[1] === "www." ? "https://" : protocolMatches[1];
			else return "https://";
		};
		const getUrl = (pattern, url) => {
			...
			if (isNonNullable(match)) for (let i = 0; i < match.length; i++) newUrl = newUrl.replace("$" + i, () => match[i] ? match[i] : "");
			return newUrl.replace(/\?$/, "");
		};
		const matchPattern = (url) => {
			const patterns = urlPatterns.filter((pattern) => pattern.regex.test(url));
			if (patterns.length > 0) return global$5.extend({}, patterns[0], { url: getUrl(patterns[0], url) });
			else return null;
		};
		const getIframeHtml = (data, iframeTemplateCallback) => {
			if (iframeTemplateCallback) return iframeTemplateCallback(data);
			else {
		...
		const getFlashHtml = (data) => {
			...
			if (data.poster) html += "<img src=\"" + data.poster + "\" width=\"" + data.width + "\" height=\"" + data.height + "\" />";
			html += "</object>";
			return html;
		};
		const getAudioHtml = (data, audioTemplateCallback) => {
			if (audioTemplateCallback) return audioTemplateCallback(data);
			else return "<audio controls=\"controls\" src=\"" + data.source + "\">" + (data.altsource ? "\n<source src=\"" + data.altsource + "\"" + (data.altsourcemime ? " type=\"" + data.altsourcemime + "\"" : "") + " />\n" : "") + "</audio>";
		};
		const getVideoHtml = (data, videoTemplateCallback) => {
			if (videoTemplateCallback) return videoTemplateCallback(data);
			else return "<video width=\"" + data.width + "\" height=\"" + data.height + "\"" + (data.poster ? " poster=\"" + data.poster + "\"" : "") + " controls=\"controls\">\n<source src=\"" + data.source + "\"" + (data.sourcemime ? " type=\"" + data.sourcemime + "\"" : "") + " />\n" + (data.altsource ? "<source src=\"" + data.altsource + "\"" + (data.altsourcemime ? " type=\"" + data.altsourcemime + "\"" : "") + " />\n" : "") + "</video>";
		};
		const dataToHtml = (editor, dataIn) => {
			...
			if (!data.altsource) data.altsource = "";
			...
			if (data.embed) return updateHtml(data.embed, data, true, editor.schema);
			else {
				...
				if (data.type === "iframe") return getIframeHtml(data, iframeTemplateCallback);
				else if (data.sourcemime === "application/x-shockwave-flash") return getFlashHtml(data);
				else if (data.sourcemime.indexOf("audio") !== -1) return getAudioHtml(data, audioTemplateCallback);
				else return getVideoHtml(data, videoTemplateCallback);
			}
		};
		...
		const setup$2 = (editor) => {
			editor.on("mousedown", (e) => {
				...
				if (previewObj && editor.dom.getAttrib(previewObj, "data-mce-selected") === "2") e.stopImmediatePropagation();
			});
			editor.on("click keyup touchend", () => {
				const selectedNode = editor.selection.getNode();
				if (selectedNode && editor.dom.hasClass(selectedNode, "mce-preview-object")) {
					if (editor.dom.getAttrib(selectedNode, "data-mce-selected")) selectedNode.setAttribute("data-mce-selected", "2");
				}
			});
		...
		const embedPromise = (data, dataToHtml, handler) => {
			return new Promise((res, rej) => {
				const wrappedResolve = (response) => {
					if (response.html) cache[data.source] = response;
					return res({
						url: data.source,
						html: response.html ? response.html : dataToHtml(data)
					});
				};
				if (cache[data.source]) wrappedResolve(cache[data.source]);
				else handler({ url: data.source }).then(wrappedResolve).catch(rej);
			});
		};
		...
		const selectPlaceholder = (editor, beforeObjects) => {
			...
			for (let i = 0; i < beforeObjects.length; i++) for (let y = afterObjects.length - 1; y >= 0; y--) if (beforeObjects[i] === afterObjects[y]) afterObjects.splice(y, 1);
		...
		const submitForm = (prevData, newData, editor) => {
			...
			if (newData.embed && (prevData.source === newData.source || isCached(newData.source))) handleInsert(editor, newData.embed);
			else getEmbedHtml(editor, newData).then((response) => {
		...
		const showDialog = (editor) => {
			...
			if (hasAltSource(editor)) advancedFormItems.push({
				name: "altsource",
				type: "urlinput",
				filetype: "media",
				label: "Alternative source URL"
			});
			if (hasPoster(editor)) advancedFormItems.push({
				name: "poster",
				type: "urlinput",
				filetype: "image",
			...
			if (advancedFormItems.length > 0) tabs.push(advancedTab);
			const body = {
				type: "tabpanel",
				tabs
			};
			const win = editor.windowManager.open({
				...
				onChange: (api, detail) => {
					switch (detail.name) {
						...
						case "dimensions":
						case "altsource":
						case "poster": handleUpdate(api, detail.name, currentData.get());
					}
		...
		const getDimension = (node, styles, dimension, defaultValue = null) => {
			...
			else if (!has(styles, dimension)) return defaultValue;
			else return null;
		};
		...
		const createPreviewNode = (editor, node) => {
			...
			if (name === "iframe") previewNode.attr({
				...
				if (isNonNullable(sanitizedHtml)) appendNodeContent(editor, name, previewNode, unescape(sanitizedHtml));
			}
		...
		const retainAttributesAndInnerHtml = (editor, sourceNode, targetNode) => {
			...
			while (ai--) {
				...
				if (attrName !== "width" && attrName !== "height" && attrName !== "style" && !startsWith(attrName, "data-mce-")) {
					if (attrName === "data" || attrName === "src") attrValue = editor.convertURL(attrValue, attrName);
		...
		const isWithinEmbedWrapper = (node) => {
			let tempNode = node;
			while (tempNode = tempNode.parent) if (tempNode.attr("data-ephox-embed-iri") || isPageEmbedWrapper(tempNode)) return true;
			return false;
		};
		const placeHolderConverter = (editor) => (nodes) => {
			...
			while (i--) {
				...
				if (node.parent.attr("data-mce-object")) continue;
				if (isLiveEmbedNode(node) && hasLiveEmbeds(editor)) {
					if (!isWithinEmbedWrapper(node)) node.replace(createPreviewNode(editor, node));
				} else if (!isWithinEmbedWrapper(node)) node.replace(createPlaceholderNode(editor, node));
			}
		};
		...
		const buildMediaElement = (editor, node) => {
			...
			if (realElmName !== "audio") {
				...
				if (className && className.indexOf("mce-preview-object") !== -1 && firstChild) {
					...
					if (isString(width)) element.setAttribute("width", width);
					if (isString(height)) element.setAttribute("height", height);
				} else {
			...
			const style = node.attr("style");
			if (isString(style)) element.setAttribute("style", style);
			const attribs = node.attributes ?? [];
			let ai = attribs.length;
			while (ai--) {
				const attrName = attribs[ai].name;
				if (attrName.indexOf("data-mce-p-") === 0) element.setAttribute(attrName.substr(11), attribs[ai].value);
			}
			const innerHtml = node.attr("data-mce-html");
			if (isString(innerHtml)) element.innerHTML = unescape(innerHtml);
			else element.innerHTML = "\xA0";
			const newElement = parseAndSanitize(editor, element.outerHTML).getAll(realElmName)[0];
			if (isNonNullable(newElement)) {
				if (!isString(innerHtml)) newElement.empty();
		...
		const setup$1 = (editor) => {
			...
			editor.on("SetContent", () => {
				...
				each$1(dom.select("span.mce-preview-object"), (elm) => {
					if (dom.select("span.mce-shim", elm).length === 0) dom.add(elm, "span", { class: "mce-shim" });
				});
			});
		};
		const setup = (editor) => {
			editor.on("ResolveName", (e) => {
				let name;
				if (e.target.nodeType === 1 && (name = e.target.getAttribute("data-mce-object"))) e.name = name;
			});
		};
