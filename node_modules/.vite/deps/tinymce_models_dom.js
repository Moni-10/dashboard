	...
	*/
	(function() {
		...
		const hasProto = (v, constructor, predicate) => {
			...
			else return v.constructor?.name === constructor.name;
		};
		const typeOf = (x) => {
			const t = typeof x;
		...
		*/
		class Optional {
			...
			*/
			getOrDie(message) {
				if (!this.tag) throw new Error(message ?? "Called getOrDie on None");
		...
		const hasNonNullableKey = (obj, key) => has$1(obj, key) && obj[key] !== void 0 && obj[key] !== null;
		const isEmpty = (r) => {
		...
		const generate$1 = (cases) => {
			...
			each$2(cases, (acase, count) => {
				...
				if (adt[key] !== void 0) throw new Error("duplicate key detected:" + key);
		...
		*/
		...
		const fromText = (text, scope) => {
			const node = (scope || document).createTextNode(text);
			return fromDom$1(node);
		};
		...
		const doDiagnose = (win, ranges) => {
			...
			if (rng.collapsed) return ranges.rtl().filter((rev) => rev.collapsed === false).map((rev) => adt$6.rtl(SugarElement.fromDom(rev.endContainer), rev.endOffset, SugarElement.fromDom(rev.startContainer), rev.startOffset)).getOrThunk(() => fromRange(win, adt$6.ltr, rng));
			else return fromRange(win, adt$6.ltr, rng);
		};
		...
		const asLtrRange = (win, selection) => {
			return diagnose(win, selection).match({
		...
		const detectBrowser$1 = (browsers, userAgentData) => {
			return findMap(userAgentData.brands, (uaBrand) => {
				...
				return find$1(browsers, (browser) => lcBrand === browser.brand?.toLowerCase()).map((info) => ({
					current: info.name,
		...
		const detect$3 = (userAgent, userAgentDataOpt, mediaMatch) => {
			...
			return {
				browser,
				os,
				deviceType: DeviceType(os, browser, userAgent, mediaMatch)
			};
		};
		...
		const getOrDie = (name, scope) => {
			...
			if (actual === void 0 || actual === null) throw new Error(name + " not available on this browser");
			return actual;
		};
		...
		*/
		...
		const unwrap = (wrapper) => {
			...
			if (children.length > 0) after$3(wrapper, children);
			remove$5(wrapper);
		};
		...
		const remove$4 = (element, property) => {
			...
			if (is$2(getOpt(element, "style").map(trim), "")) remove$6(element, "style");
		};
		...
		const Dimension = (name, getOffset) => {
			...
			const max = (element, value, properties) => {
				...
				return value > cumulativeInclusions ? value - cumulativeInclusions : 0;
			};
			return {
				set,
				get,
		...
		const viewport = (element) => {
			const dom = element.dom;
			const body = dom.ownerDocument.body;
		...
		const read = (element, attr) => {
			...
			return value === void 0 || value === "" ? [] : value.split(" ");
		};
		const add$3 = (element, attr, id) => {
			const nu = read(element, attr).concat([id]);
		...
		const add$1 = (element, clazz) => {
			...
			else add$2(element, clazz);
		};
		const cleanClass = (element) => {
			if ((supports(element) ? element.dom.classList : get$4(element)).length === 0) remove$6(element, "class");
		};
		const remove$1 = (element, clazz) => {
			if (supports(element)) element.dom.classList.remove(clazz);
			else remove$2(element, clazz);
			cleanClass(element);
		};
		...
		const ancestors$3 = (scope, selector, isRoot) => ancestors$4(scope, (e) => is$1(e, selector), isRoot);
		const children = (scope, selector) => children$1(scope, (e) => is$1(e, selector));
		...
		const SimRange = { create: create$4 };
		...
		const caretPositionFromPoint = (doc, x, y) => Optional.from(doc.caretPositionFromPoint?.(x, y)).bind((pos) => {
		...
		const availableSearch = (doc, x, y) => {
			if (doc.caretPositionFromPoint) return caretPositionFromPoint(doc, x, y);
			else if (doc.caretRangeFromPoint) return caretRangeFromPoint(doc, x, y);
			else return Optional.none();
		};
		...
		const setToElement = (win, element, selectNodeContents$1 = true) => {
			const rng = (selectNodeContents$1 ? selectNodeContents : selectNode)(win, element);
			doSetNativeRange(win, rng);
		};
		...
		const pattern = (() => {
			...
			const float = `[+-]?(?:${[
				"Infinity",
				"[0-9]+\\." + opt(decimalDigits) + opt(exponentPart),
				"\\.[0-9]+" + opt(exponentPart),
		...
		const columns$1 = (ancestor) => {
			if (is$1(ancestor, "colgroup")) return children(ancestor, "col");
			else return bind$2(columnGroups(ancestor), (columnGroup) => children(columnGroup, "col"));
		};
		...
		const fromTable$1 = (table) => {
			...
			const elems = [...columnGroups(table), ...rows];
		...
		const cells = () => ({
			transformRow: (row, section) => {
				return transformRow(row, section === "thead" ? "tbody" : section);
			},
			transformCell
		});
		const fallback = () => ({
		...
		const getTableSectionType = (table, fallback) => {
			...
			switch (findTableRowHeaderType(warehouse).getOr(fallback)) {
		...
		const decide = (getBlock, isValid, getFallback) => {
			...
			return find$1(inBlock, isValid).orThunk(() => Optional.from(inBlock[0]).orThunk(getFallback)).map((detail) => detail.element);
		};
		...
		const rtlEdge = (cell) => {
			return absolute(cell).left + getOuter(cell);
		};
		const ltrEdge = (cell) => {
		...
		const get$2 = (cell, type, f) => {
			return f(cell) / getSpan(cell, type);
		};
		...
		const getPixelWidth$1 = (cell) => isCol$2(cell) ? Math.round(get$7(cell)) : getRuntime(cell);
		const getHeight = (cell) => {
			return isRow$2(cell) ? get$8(cell) : get$2(cell, "rowspan", getTotalHeight);
		};
		const getGenericWidth = (cell) => {
			return getRawWidth$1(cell).bind((w) => parse(w, [
				"fixed",
				"relative",
				"empty"
			]));
		...
		const percentageSize = (table) => {
			...
			const adjustTableWidth = (delta) => {
				...
				const newWidth = currentWidth + delta / 100 * currentWidth;
		...
		const chooseSize = (element, width) => {
			if (percentageBasedSizeRegex().exec(width) !== null) return percentageSize(element);
			else return pixelSize(element);
		};
		const getTableSize = (table) => {
			return getRawWidth$1(table).fold(() => noneSize(table), (w) => chooseSize(table, w));
		};
		const TableSize = {
			getTableSize,
			pixelSize,
		...
		const setIfNot = (element, property, value, ignore) => {
			if (value === ignore) remove$6(element, property);
		...
		const render$1 = (table, grid) => {
			...
			const renderSection = (gridSection, sectionName) => {
				...
				const sectionElems = (sectionName === "colgroup" ? syncColGroup : syncRows)(gridSection);
			...
			each$2(grid, (row) => {
				switch (row.section) {
					...
					case "tfoot":
						...
						break;
					case "colgroup": columnGroupsSection.push(row);
				}
			});
		...
		const subgrid = (grid, row, column, comparator) => {
			...
			return {
				...
				rowspan: isColRow ? 1 : findDiff(getColumn(grid.slice(row), column), comparator)
			};
		};
		...
		const run = (operation, extract, adjustment, postAction, genWrappers, table, target, generators, behaviours) => {
			...
			return extract(warehouse, target).map((info) => {
		...
		const total = (start, end, measures) => {
			let r = 0;
			for (let i = start; i < end; i++) r += measures[i] !== void 0 ? measures[i] : 0;
			return r;
		};
		...
		const constrainSpan = (element, property, value) => {
			...
			if (value === 1 || currentColspan <= 1) remove$6(element, property);
		...
		const copyRows = (table, target, generators) => {
			...
			return onCells(warehouse, target).bind((selectedCells) => {
				...
				const slicedGrid = extractGridDetails(grid).rows.slice(selectedCells[0].row, selectedCells[selectedCells.length - 1].row + selectedCells[selectedCells.length - 1].rowspan);
		...
		const makeCell = (list, seenSelected, rowIndex) => {
			...
			(seenSelected ? append$1 : prepend)(row, td);
		};
		const fillInGaps = (list, house, stats, isSelected) => {
			...
			for (let i = 0; i < totalRows; i++) {
				...
				for (let j = 0; j < totalColumns; j++) if (!(i < stats.minRow || i > stats.maxRow || j < stats.minCol || j > stats.maxCol)) {
					if (Warehouse.getAt(house, i, j).filter(isSelected).isNone()) makeCell(rows, seenSelected, i);
		...
		const clean = (replica, stats, house, widthDelta) => {
			each$1(house.columns, (col) => {
				if (col.column < stats.minCol || col.column > stats.maxCol) remove$5(col.element);
			});
		...
		const getTableWidthDelta = (table, warehouse, tableSize, stats) => {
			...
			const delta = foldl(colWidths.slice(stats.minCol, stats.maxCol + 1), (acc, width) => acc + width, 0) / allColsWidth * tableSize.pixelWidth() - tableSize.pixelWidth();
		...
		const getOtherCells = (table, target, generators) => {
			...
			return onCells(warehouse, target).map((selectedCells) => {
				...
				return {
					upOrLeftCells: getUpOrLeftCells(rows, selectedCells),
					downOrRightCells: getDownOrRightCells(rows, selectedCells)
				};
			});
		};
		...
		const from = (value) => {
			if (endsWith(value, "%")) return validateFor("%", adt$2.percent, value);
			if (endsWith(value, "px")) return validateFor("px", adt$2.pixels, value);
		...
		const advance = (universe, item, direction, transition = advance) => {
			...
			return direction.first(children).map((r) => {
		...
		const hone = (universe, item, predicate, mode, direction, isRoot) => {
			return go(universe, item, mode, direction).bind((n) => {
		...
		const toEnd = (universe, element) => {
			...
			return universe.property().children(element).length;
		};
		...
		const all = (universe, look, elements, f) => {
			...
			return f(universe, look, head, elements.slice(1));
		};
		/**
		...
		*/
		...
		const ancestors$2 = (universe, start, end, isRoot = never) => {
			...
			const prune = (path) => {
				return findIndex(path, isRoot).fold(() => {
					return path;
				}, (ind) => {
		...
		const cellOperations = (mutate, doc, formatsToClone) => {
			const cloneCss = (prev, clone) => {
				...
				if (prev.colspan !== 1) remove$4(clone, "width");
			};
			const newCell = (prev) => {
		...
		const getGridSize = (table) => {
			return Warehouse.fromTable(table).grid;
		};
			...
			if (index > 0 && index < rows.length) {
				...
				each$2(cells, (cell) => {
					...
					for (let i = index; i < rows.length; i++) for (let j = 0; j < cellLength(rows[0]); j++) {
						...
						if (comparator(current.element, cell.element)) {
		...
		const isSpanning = (grid, row, col, comparator) => {
			const matching = curry(comparator, getCell(grid[row], col).element);
		...
		const mergeTables = (startAddress, gridA, gridBRows, generator, comparator, lockedColumns) => {
			...
			for (let r = startRow; r < endRow; r++) {
				...
				for (let c = startCol; c < endCol; c++) {
					...
					if (isSpanning(gridA, r, c, comparator)) unmerge(gridA, getCellElement(gridA[r], c), comparator, generator.cell);
		...
		const getValidStartAddress = (currentStartAddress, grid, lockedColumns) => {
			...
			return {
				...
				column: find$1(possibleColAddresses, (num) => forall(lockedColumns, (col) => col !== num)).getOr(gridColLength - 1)
			};
		};
		...
		const insertRowAt = (grid, index, example, comparator, substitution) => {
			...
			const newRow = cloneRow(rows[example], (ex, c) => {
				return index > 0 && index < rows.length && comparator(getCellElement(rows[index - 1], c), getCellElement(rows[index], c)) ? getCell(rows[index], c) : elementnew(substitution(ex.element, comparator), true, ex.isLocked);
		...
		const notInStartRow = (grid, rowIndex, colIndex, comparator) => getCellElement(grid[rowIndex], colIndex) !== void 0 && rowIndex > 0 && comparator(getCellElement(grid[rowIndex - 1], colIndex), getCellElement(grid[rowIndex], colIndex));
		...
		const rowReplacerPredicate = (targetRow, columnHeaders) => {
			return forall(columnHeaders, identity) && isHeaderCells(targetRow.cells) ? always : (cell, _rowIndex, colIndex) => {
				return !(name(cell.element) === "th" && columnHeaders[colIndex]);
			};
		};
		const columnReplacePredicate = (targetColumn, rowHeaders) => {
			return forall(rowHeaders, identity) && isHeaderCells(targetColumn) ? always : (cell, rowIndex, _colIndex) => {
				return !(name(cell.element) === "th" && rowHeaders[rowIndex]);
			};
		};
		const determineScope = (applyScope, cell, newScope, isInHeader) => {
			...
			else if (isInHeader && isHeaderCell(cell)) return getScope(newScope === "row" ? "col" : "row");
			else return null;
		};
		...
		const uniqueColumns = (details) => {
			const uniqueCheck = (rest, detail) => {
				return exists(rest, (currentDetail) => currentDetail.column === detail.column) ? rest : rest.concat([detail]);
			};
			return foldl(details, uniqueCheck, []).sort((detailA, detailB) => detailA.column - detailB.column);
		};
		const isEditable = (elem) => isEditable$1(elem, true);
		const prune = (table) => {
			if (cells$1(table).length === 0) remove$5(table);
		};
		...
		const elementFromGrid = (grid, row, column) => {
			const rows = extractGridDetails(grid).rows;
			return Optional.from(rows[row]?.cells[column]?.element).filter(isEditable).orThunk(() => findEditableCursorPosition(rows));
		};
		...
		const uniqueRows = (details) => {
			const rowCompilation = (rest, detail) => {
				return exists(rest, (currentDetail) => currentDetail.row === detail.row) ? rest : rest.concat([detail]);
			};
		...
		const opInsertRowsBefore = (grid, details, comparator, genWrappers) => {
			...
			const newGrid = foldr(rows, (acc, row) => {
				return {
					grid: insertRowAt(acc.grid, targetIndex, row.row + acc.delta, comparator, genWrappers.getOrInit),
					delta: acc.delta + 1
				};
			}, {
				grid,
		...
		const opInsertColumnsBefore = (grid, extractDetail, comparator, genWrappers) => {
			...
			const newGrid = foldr(columns, (acc, col) => {
				return {
					grid: insertColumnAt(acc.grid, targetIndex, col.column + acc.delta, comparator, genWrappers.getOrInit),
					delta: acc.delta + 1
				};
			}, {
				grid,
		...
		const opPasteCells = (grid, pasteDetails, comparator, _genWrappers) => {
			...
			return merge(startAddress, grid, gridB, pasteDetails.generators, comparator).fold(() => outcome(grid, Optional.some(pasteDetails.element)), (newGrid) => {
		...
		const opGetCellsType = (table, target) => {
			const house = Warehouse.fromTable(table);
			return onCells(house, target).bind(findCommonCellType).getOr("");
		};
		...
		const getColumnsWidth = (warehouse, details) => {
			...
			return foldl(uniqueCols, (acc, detail) => {
				return acc + columns$1[detail.column].map(getOuter).getOr(0);
			}, 0);
		};
		const insertColumnsExtractor = (before) => (warehouse, target) => onCells(warehouse, target).filter((details) => {
			return !(before ? firstColumnIsLocked : lastColumnIsLocked)(warehouse, details);
		}).map((details) => ({
			details,
		...
		const pasteColumnsExtractor = (before) => (warehouse, target) => onPasteByEditor(warehouse, target).filter((details) => {
			return !(before ? firstColumnIsLocked : lastColumnIsLocked)(warehouse, details.cells);
		});
		...
		const moveBy$1 = (warehouse, cell, row, column) => {
			return Warehouse.findItem(warehouse, cell, eq$1).bind((detail) => {
				...
				return Warehouse.getAt(warehouse, startRow + row, startCol + column).map((d) => {
					return d.element;
				});
			});
		};
		...
		const render = (rows, columns, rowHeaders, columnHeaders, headerType, renderOpts = DefaultRenderOptions) => {
			...
			if (rowHeadersGoInThead && rowHeaders > 0) {
				...
				const theadRows = createRows(rowHeaders, columns, headerType === "sectionCells" ? actualRowHeaders : 0, columnHeaders);
				append(thead, theadRows);
			}
			const tbody = SugarElement.fromTag("tbody");
			append$1(table, tbody);
			const numRows = rowHeadersGoInThead ? rows - actualRowHeaders : rows;
			const tbodyRows = createRows(numRows, columns, rowHeadersGoInThead ? 0 : rowHeaders, columnHeaders);
		...
		const InDrag = () => {
			...
			const onEvent = (event, mode) => {
				mode.extract(event).each((data) => {
					update(mode, data).each((d) => {
						events.trigger.move(d);
					});
				});
			};
			const events = create$3({ move: Event(["info"]) });
			return {
				onEvent,
				reset,
		...
		const Mutation = () => {
			const events = create$3({ drag: Event(["xDelta", "yDelta"]) });
			const mutate = (x, y) => {
		...
		const BarMutation = () => {
			const events = create$3({ drag: Event([
				"xDelta",
				"yDelta",
				"target"
			]) });
		...
		const resizableColumns = (warehouse, isResizable) => {
			...
			range$1(warehouse.grid.columns, (index) => {
				if (Warehouse.getColumnAt(warehouse, index).map((col) => col.element).forall(isResizable)) resizableCols.push(index);
			});
		...
		const BarManager = (wire) => {
			...
			const mouseover = bind(wire.view(), "mouseover", (event) => {
				findClosestEditableTable(event.target).fold(() => {
					if (inBody(event.target) && !isResizer(event.target)) destroy(wire);
				}, (table) => {
		...
		const TableResize = { create: create$2 };
		...
		const Response = { create: create$1 };
		...
		const verify = (bridge, before, beforeOffset, after, afterOffset, failure, isRoot) => {
			return closest$1(after, "td,th", isRoot).bind((afterCell) => {
				return closest$1(before, "td,th", isRoot).map((beforeCell) => {
					...
					else return eq$1(after, afterCell) && getEnd(afterCell) === afterOffset ? failure(beforeCell) : adt$1.none("in same cell");
				});
		...
		const adjustDown = (bridge, element, guessBox, original, caret) => {
			...
			else if (guessBox.top === caret.bottom) return adt.retry(moveDown(caret, 1));
			else return inOutsideBlock(bridge, element, caret) ? adt.retry(translate(lowerCaret, JUMP_SIZE, 0)) : adt.none();
		};
		const adjustUp = (bridge, element, guessBox, original, caret) => {
			...
			else if (guessBox.bottom === caret.top) return adt.retry(moveUp(caret, 1));
			else return inOutsideBlock(bridge, element, caret) ? adt.retry(translate(higherCaret, JUMP_SIZE, 0)) : adt.none();
		};
		const upMovement = {
			point: getTop,
			adjuster: adjustUp,
		...
		const scan = (bridge, isRoot, element, offset, direction, numRetries) => {
			...
			return tryCursor(bridge, isRoot, element, offset, direction).bind((situs) => {
				...
				return BeforeAfter.cata(analysis, () => {
					...
					else return scan(bridge, isRoot, cell, getEnd(cell), direction, numRetries - 1);
				});
			});
		};
					...
					findCell(event.target, isRoot).each((finish) => {
						identify(start, finish, isRoot).each((cellSel) => {
							...
							if (boxes.length === 1) {
								...
								if (isNonEditableCell && isCellClosestContentEditable) annotations.selectRange(container, boxes, singleCell, singleCell);
		...
		const WindowBridge = (win) => {
			...
			const getScrollY = () => {
				return get$6(SugarElement.fromDom(win.document)).top;
			};
			const scrollBy = (x, y) => {
			...
			editor.on("BeforeGetContent", (e) => {
				const multiCellContext = (cells) => {
					...
					extractSelected(cells).each((replicaElements) => {
						e.content = e.format === "text" ? getTextContent(editor, replicaElements) : serializeElements(editor, replicaElements);
					});
				};
				if (e.selection === true) {
		...
		const TableActions = (editor, resizeHandler, cellSelectionHandler) => {
			...
			const getTableSectionType = (table) => {
				switch (getTableHeaderType(editor)) {
					...
					default: return TableSection.getTableSectionType(table, "section");
				}
			};
			...
			const execute = (operation, guard, mutate, effect) => (table, target, noEvents = false) => {
				...
				return guard(table) ? operation(table, target, generators, behaviours).bind((result) => {
					...
					if (inBody(table)) {
						removeDataStyle(table);
						if (!noEvents) fireTableModified(editor, table.dom, effect);
					}
		...
		const insert = (editor, columns, rows, colHeaders, rowHeaders) => {
			...
			return descendant(getBody(editor), "table[data-mce-id=\"__mce\"]").map((table) => {
				if (isTablePixelsForced(editor)) convertToPixelSizeWidth(table);
				else if (isTableResponsiveForced(editor)) convertToNoneSizeWidth(table);
				else if (isTablePercentagesForced(editor) || isPercentage(defaultStyles.width)) convertToPercentSizeWidth(table);
				removeDataStyle(table);
		...
		const registerCommands = (editor, actions) => {
			...
			const setSizingMode = (sizing) => getSelectionStartCellOrCaption(editor).each((cellOrCaption) => {
				if (!(isTableResponsiveForced(editor) || isTablePixelsForced(editor) || isTablePercentagesForced(editor))) table(cellOrCaption, isRoot).each((table) => {
					if (sizing === "relative" && !isPercentSizing(table)) convertToPercentSizeWidth(table);
					else if (sizing === "fixed" && !isPixelSizing(table)) convertToPixelSizeWidth(table);
					else if (sizing === "responsive" && !isNoneSizing(table)) convertToNoneSizeWidth(table);
					removeDataStyle(table);
			...
			const toggleTableCellClass = (_ui, clazz) => {
				performActionOnSelection((table) => {
					const selectedCells = getCellsFromSelection(editor);
					const formatterAction = forall(selectedCells, (cell) => editor.formatter.match("tablecellclass", { value: clazz }, cell.dom)) ? editor.formatter.remove : editor.formatter.apply;
			...
			const actOnSelection = (execute, noEvents = false) => performActionOnSelection((table, startCell) => {
				execute(table, forMenu(getCellsFromSelection(editor), table, startCell), noEvents).each(postExecute);
			});
			...
			const pasteOnSelection = (execute, getRows) => getRows().each((rows) => {
				...
				performActionOnSelection((table, startCell) => {
					...
					execute(table, pasteRows(getCellsFromSelection(editor), startCell, clonedRows, generators)).each(postExecute);
				});
			});
			...
			editor.addCommand("mceTableApplyCellStyle", (_ui, args) => {
				...
				each$1(validArgs, (value, style) => {
					...
					each$2(cells, (cell) => {
						if (value === "") editor.formatter.remove(formatName, { value: null }, cell.dom, true);
						else editor.formatter.apply(formatName, { value }, cell.dom);
					});
				});
		...
		const registerQueryCommands = (editor, actions) => {
			...
			const lookupOnSelection = (action) => getSelectionCell(getSelectionStart(editor)).bind((cell) => table(cell, isRoot).map((table) => {
				return action(table, forMenu(getCellsFromSelection(editor), table, cell));
			})).getOr("");
			each$1({
		...
		const TableCellSelectionHandler = (editor, resizeHandler) => {
			...
			editor.on("init", (_e) => {
				...
				const syncSelection = () => {
					...
					sharedOne(table, [start, end]).fold(() => annotations.clear(body), noop);
				};
		...
		const TableResizeHandler = (editor) => {
			...
			const afterCornerResize = (table, origin, width, height) => {
				...
				if (startRawW === "") convertToPercentSizeWidth(table);
				if (startRawH === "") convertToPixelSizeHeight(table);
				if (width !== startW && startRawW !== "") {
					...
					const targetPercentW = width * parseFloat(startRawW.replace("%", "")) / startW;
					set$1(table, "width", targetPercentW + "%");
				}
				if (isPixel(startRawW)) syncTableCellPixels(table);
			...
			editor.on("init", () => {
				...
				if (hasTableObjectResizing(editor) && hasTableResizeBars(editor)) {
					...
					if (!editor.mode.isReadOnly()) sz.on();
			...
			editor.on("ObjectResizeStart", (e) => {
				...
				if (isTable(targetElm) && !editor.mode.isReadOnly()) {
					...
					if (!isPixelSizing(table) && isTablePixelsForced(editor)) convertToPixelSizeWidth(table);
					else if (!isPercentSizing(table) && isTablePercentagesForced(editor)) convertToPercentSizeWidth(table);
					if (isNoneSizing(table) && startsWith(e.origin, barResizerPrefix)) convertToPercentSizeWidth(table);
					startW = e.width;
			...
			editor.on("ObjectResized", (e) => {
				...
				if (isTable(targetElm)) {
					...
					if (isCornerResize(origin)) afterCornerResize(table, origin, e.width, e.height);
					removeDataStyle(table);
					fireTableModified(editor, table.dom, styleModified);
				}
			});
