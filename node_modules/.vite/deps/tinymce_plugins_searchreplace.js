...
//#endregion
//#region node_modules/tinymce/plugins/searchreplace/index.js
		...
		*/
		...
		const walk = (dom, walkerFn, startNode, callbacks, endNode, skipStart = true) => {
			...
			while (next) {
				...
				if (isCefNode || isHidden(dom, next)) {
					if (isCefNode ? callbacks.cef(next) : callbacks.boundary(next)) break;
					else {
						next = walkerFn(true);
						continue;
					}
				} else if (isSimpleBoundary(dom, next)) {
					if (callbacks.boundary(next)) break;
				} else if (isText(next)) callbacks.text(next);
				if (next === endNode) break;
				else next = walkerFn(false);
			}
		};
		const collectTextToBoundary = (dom, section, node, rootNode, forwards) => {
			if (isBoundary(dom, node)) return;
			const walker = new global(node, dom.getParent(rootNode, dom.isBlock) ?? dom.getRoot());
			const walkerFn = forwards ? walker.next.bind(walker) : walker.prev.bind(walker);
			walk(dom, walkerFn, node, {
				boundary: always,
				cef: always,
				text: (next) => {
					if (forwards) section.fOffset += next.length;
					else section.sOffset += next.length;
		...
		const collect = (dom, rootNode, startNode, endNode, callbacks, skipStart = true) => {
			...
			walk(dom, walker.next.bind(walker), startNode, {
				...
				cef: (node) => {
					finishSection();
					if (callbacks) sections.push(...callbacks.cef(node));
					return false;
				},
				text: (next) => {
					current.elements.push(SugarElement.fromDom(next));
					if (callbacks) callbacks.text(next, current);
				}
			}, endNode, skipStart);
			if (endNode) collectTextToBoundary(dom, current, endNode, rootNode, true);
			finishSection();
			return sections;
		};
		const collectRangeSections = (dom, rng) => {
			...
			return collect(dom, rng.commonAncestorContainer, startNode, endNode, {
				text: (node, section) => {
					if (node === endNode) section.fOffset += node.length - end.offset;
					else if (node === startNode) section.sOffset += start.offset;
				},
				cef: (node) => {
		...
		const extract = (elements, matches) => {
			const nodePositions = foldl(elements, (acc, element) => {
				...
				const positions = bind(matches, (match, matchIdx) => {
					if (match.start < finish && match.finish > start) return [{
						element,
		...
		const mark = (matches, replacementNode) => {
			eachr(matches, (match, idx) => {
				eachr(match, (pos) => {
					...
					if (textNode.length === pos.finish && pos.start === 0) wrap(pos.element, wrapper);
					else {
						if (textNode.length !== pos.finish) textNode.splitText(pos.finish);
		...
		const markAllMatches = (editor, currentSearchState, pattern, inSelection) => {
			...
			if (inSelection) return findAndMarkInSelection(editor.dom, pattern, editor.selection, marker);
			else return findAndMark(editor.dom, pattern, node, marker);
		};
		const unwrap = (node) => {
			const parentNode = node.parentNode;
			if (node.firstChild) parentNode.insertBefore(node.firstChild, node);
			node.parentNode?.removeChild(node);
		};
		const findSpansByIndex = (editor, index) => {
			const spans = [];
			const nodes = global$1.toArray(editor.getBody().getElementsByTagName("span"));
			if (nodes.length) for (let i = 0; i < nodes.length; i++) {
				const nodeIndex = getElmIndex(nodes[i]);
				if (nodeIndex === null || !nodeIndex.length) continue;
		...
		const moveSelection = (editor, currentSearchState, forward) => {
			...
			if (forward) {
				if (testIndex + 1 === searchState.count) testIndex = 0;
				else testIndex++;
			} else if (testIndex - 1 === -1) testIndex = searchState.count - 1;
			else testIndex--;
		...
		const removeNode = (dom, node) => {
			...
			dom.remove(node);
			if (parent && dom.isEmpty(parent)) dom.remove(parent);
		};
		const escapeSearchText = (text, wholeWord) => {
			const wordRegex = "(" + text.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&").replace(/\s/g, "[^\\S\\r\\n\\uFEFF]") + ")";
		...
		const find = (editor, currentSearchState, text, matchCase, wholeWord, inSelection) => {
			...
			if (global$2.browser.isSafari()) selection.setRng(selection.getRng(), isForwardSelection);
			if (count) {
		...
		const replace = (editor, currentSearchState, text, forward, all) => {
			...
			for (let i = 0; i < nodes.length; i++) {
				...
				if (all || matchIndex === searchState.index) {
					if (text.length) {
						...
						unwrap(nodes[i]);
					} else removeNode(editor.dom, nodes[i]);
					while (nodes[++i]) {
						matchIndex = parseInt(getElmIndex(nodes[i]), 10);
						if (matchIndex === currentMatchIndex) removeNode(editor.dom, nodes[i]);
						else {
							i--;
							break;
						}
				...
				} else if (currentMatchIndex > currentIndex) nodes[i].setAttribute("data-mce-index", String(currentMatchIndex - 1));
			}
			currentSearchState.set({
				...searchState,
			...
			if (forward) next(editor, currentSearchState);
			else prev(editor, currentSearchState);
		...
		const done = (editor, currentSearchState, keepEditorSelection) => {
			...
			for (let i = 0; i < nodes.length; i++) {
				...
				if (nodeIndex !== null && nodeIndex.length) {
					if (nodeIndex === searchState.index.toString()) {
						if (!startContainer) startContainer = nodes[i].firstChild;
			...
			if (startContainer && endContainer) {
				...
				if (keepEditorSelection !== false) editor.selection.setRng(rng);
				return rng;
			} else return;
		};
		...
		const open = (editor, currentSearchState) => {
			...
			const focusButtonIfRequired = (api, name) => {
				if (global$2.browser.isSafari() && global$2.deviceType.isTouch() && (name === "find" || name === "replace" || name === "replaceall")) api.focus(name);
			};
			const reset = (api) => {
				done(editor, currentSearchState, false);
				disableAll(api, true);
			...
			const doFind = (api) => {
				...
				if (last.text === data.findtext && last.matchCase === data.matchcase && last.wholeWord === data.wholewords) next(editor, currentSearchState);
				else {
					const count = find(editor, currentSearchState, data.findtext, data.matchcase, data.wholewords, data.inselection);
					if (count <= 0) toggleNotFoundAlert(true, api);
			...
			const getDialogSpec = (showNoMatchesAlertBanner, initialData) => ({
				...
				onChange: (api, details) => {
					if (showNoMatchesAlertBanner) toggleNotFoundAlert(false, api);
					if (details.name === "findtext" && currentSearchState.get().count > 0) reset(api);
				},
				onAction: (api, details) => {
					...
					switch (details.name) {
						case "find":
							...
							break;
						case "replace":
							if (!replace(editor, currentSearchState, data.replacetext)) reset(api);
							else updateButtonStates(api);
							break;
						case "replaceall":
