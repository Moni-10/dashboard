...
//#endregion
//#region node_modules/tinymce/plugins/emoticons/index.js
	...
	(function() {
		...
		*/
		...
		const emojisFrom = (list, pattern, maxResults) => {
			...
			for (let i = 0; i < list.length; i++) if (pattern.length === 0 || emojiMatches(list[i], lowerCasePattern)) {
				matches.push({
					value: list[i].char,
					text: list[i].title,
					icon: list[i].char
				});
				if (maxResults.exists((max) => matches.length >= max)) break;
			}
			return matches;
		};
