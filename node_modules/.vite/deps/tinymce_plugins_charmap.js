...
//#endregion
//#region node_modules/tinymce/plugins/charmap/index.js
	...
	(function() {
		...
		*/
		...
		const getCharsFromOption = (optionValue) => {
			if (isArray(optionValue)) return charmapFilter(optionValue);
			if (typeof optionValue === "function") return optionValue();
			return [];
		};
		const extendCharMap = (editor, charmap) => {
			const userCharMap = getCharMap$1(editor);
			if (userCharMap) charmap = [{
				name: UserDefined,
		...
		const charMatches = (charCode, name, lowerCasePattern) => {
			if (contains(fromCodePoint(charCode).toLowerCase(), lowerCasePattern)) return true;
			else return contains(name.toLowerCase(), lowerCasePattern) || contains(name.toLowerCase().replace(/\s+/g, ""), lowerCasePattern);
		};
		const scan = (group, pattern) => {
			const matches = [];
			const lowerCasePattern = pattern.toLowerCase();
			each(group.characters, (g) => {
				if (charMatches(g[0], g[1], lowerCasePattern)) matches.push(g);
			});
		...
		const open = (editor, charMap) => {
			...
			const bridgeSpec = {
				...
				size: "normal",
				body: charMap.length === 1 ? makePanel() : makeTabPanel(),
				buttons: [{
					type: "cancel",
					name: "close",
					text: "Close",
				...
				onChange: (dialogApi, changeData) => {
					if (changeData.name === patternName) updateFilter.throttle(dialogApi);
				}
			};
			editor.windowManager.open(bridgeSpec).focus(patternName);
		};
