	...
	(function() {
		...
		*/
		...
		const register$1 = (editor) => {
			...
			registerOption("files_upload_handler", { processor: "function" });
		...
		const linkDomMutation = (editor, attachState, data) => {
			...
			if (isNonNullable(attributesPostProcess$1)) attributesPostProcess$1(linkAttrs);
		...
		const tryProtocolTransform = (assumeExternalTargets, defaultLinkProtocol) => (data) => {
			const url = data.href;
			return assumeExternalTargets === 1 && !hasProtocol(url) || assumeExternalTargets === 0 && /^\s*www(\.|\d\.)/i.test(url) ? Optional.some({
		...
		const uploadFile = (editor) => (blobInfo, progress) => {
			return getFilesUploadHandler(editor)(blobInfo, progress);
		};
		...
		const makeDialog = (settings, onSubmit, editor) => {
			...
			return {
				...
				size: "normal",
				body: makeDialogBody(urlInput, displayText, titleText, catalogs, settings.hasUploadPanel, getDocumentsFileTypes(editor), () => new Promise((r) => helpers.alertErr("Selected files do not have allowed extensions", r))),
				buttons: [{
					type: "cancel",
					name: "cancel",
					text: "Cancel"
				...
				initialData,
				onChange: (api, { name }) => {
					if (name === "fileinput") changeFileInput(helpers, api);
					else dialogDelta.onChange(api.getData, { name }).each((newData) => {
						api.setData(newData);
					});
				},
				onSubmit
		...
		const register = (editor) => {
			editor.addCommand("mceLink", (_ui, value) => {
				if (value?.dialog === true || !useQuickLink(editor)) open(editor);
		...
		const setup$1 = (editor) => {
			...
			editor.on("SelectionChange", () => {
				if (!selectedLink.isSet()) getLinkFromSelection(editor).each(selectedLink.set);
			});
			editor.on("click", (e) => {
				selectedLink.clear();
			...
			editor.on("keydown", (e) => {
				selectedLink.clear();
				if (!e.isDefaultPrevented() && e.keyCode === 13 && hasOnlyAltModifier(e)) getSelectedLink().each((link) => {
					e.preventDefault();
